...
def write_email_to_card():
    try:
        conn = wait_for_card()
        print(f"[INFO] Writing email length ({EMAIL_LEN}) to block 0x41...")
        conn.transmit([0x00, 0xD6, 0x00, 0x41, 0x01, EMAIL_LEN])

        print(f"[INFO] Writing email '{EMAIL}' to block 0x100...")
        offset = 0
